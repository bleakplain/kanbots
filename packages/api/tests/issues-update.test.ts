...
describe('PATCH /api/issues/:n', () => {
  ...
  it('propagates upstream 404', async () => {
    const { app } = makeTestApp();
    const res = await request(app).patch('/api/issues/9999').send({ labels: ['x'] });
  ...
  it('propagates upstream 403', async () => {
    const { app, client } = makeTestApp();
    client.setIssue(issueFixture(7, 'lucky'));
    client.failUpdate(403, 'forbidden');
    const res = await request(app).patch('/api/issues/7').send({ labels: ['x'] });
